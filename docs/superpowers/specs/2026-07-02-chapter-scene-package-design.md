...
`Scene Category` replaces the old bare `Scene` / `ScenePack` concept in UI and code naming, while persisted legacy paths can remain compatible during migration. The previous versioned prompt/image-iteration authoring workflow is not part of the new workflow; prompt history is captured through uploaded-image snapshots instead of a separate prompt history system.
...
- `promptSnapshot`, generated by the backend from the current projected prompt at upload time. Upload UI/API does not accept a separate prompt-snapshot text override, so prompt facts keep one authority.
...
- Runs ready: at least one Complete Scene Image has been sent to the pipeline and has a recorded run status.
- Assembly ready: one Empty Scene Image is selected, at least one Scene Asset is placed, target objects are covered by linked and placed Chapter Assets, all placements have valid transforms and layer order, no placement references a deleted asset, and the Final Chapter Scene can be previewed from the assembly.
...
Run Asset materialization is deferred until the workspace has a real, selectable, verifiable run-asset source. The Chapter Scene Studio must not expose hand-entered run asset ids, fake pipeline lineage, or an Add-from-Run action that cannot prove the selected source exists.

Directly uploaded Scene Assets enter the Chapter Asset Pool immediately with direct-upload lineage. When a future Run Asset picker exists, adding a run asset may materialize a chapter-owned copy with lineage back to that verified source.

Chapter Asset deletion only affects the chapter-owned asset. If the asset is used in assembly, deleting it requires confirmation and removes the corresponding placement and dependency references.
...
- Add historical run asset back to current chapter, once a verified Run Asset picker exists.
...
- Legacy versioned prompt/image-iteration workflow.
