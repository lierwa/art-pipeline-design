...
def test_codex_cli_provider_runs_codex_exec_with_semantic_context_images(tmp_path: Path) -> None:
    ...
    raw_output_path = tmp_path / "raw.png"
    calls: list[list[str]] = []
    ...
    def runner(args: list[str], **kwargs: Any) -> subprocess.CompletedProcess[str]:
        ...
        Image.new("RGBA", (4, 4), (0, 255, 0, 255)).save(raw_output_path, format="PNG")
...
def test_codex_cli_provider_copies_only_the_current_thread_generated_image(tmp_path: Path) -> None:
    reference_path = tmp_path / "reference.png"
    source_crop_path = tmp_path / "source_crop.png"
    mask_path = tmp_path / "mask.png"
    Image.new("RGBA", (4, 4), (255, 0, 0, 255)).save(reference_path, format="PNG")
    ...
    stale_thread = generated_root / "old_thread"
    current_thread = generated_root / "thread_123"
    stale_thread.mkdir(parents=True)
    current_thread.mkdir(parents=True)
    Image.new("RGB", (4, 4), (255, 0, 0)).save(stale_thread / "old.png", format="PNG")
    Image.new("RGB", (4, 4), (0, 255, 0)).save(current_thread / "new.png", format="PNG")
    raw_output_path = tmp_path / "raw.png"
    ...
    assert result["codexThreadId"] == "thread_123"
    assert result["timing"]["rawOutputSeconds"] is not None
