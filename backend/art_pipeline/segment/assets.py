...
from art_pipeline.repair.parent_contracts import (
...
from art_pipeline.segment.quality import (
...
def suggest_sam2_edge_mask(
    ...
    mask, child_mask_subtracted = subtract_child_masks_from_parent_mask(
        workspace_root,
        output_element,
        mask,
        state,
    )
    if child_mask_subtracted:
        quality = quality_metadata_for_mask(
            mask,
            str(quality.get("selectedProfile", "base")),
            int(quality.get("candidateCount", 1)),
            int(quality.get("filledHoleCount", 0)),
            int(quality.get("filledHoleArea", 0)),
        )
...
def subtract_child_masks_from_parent_mask(
    workspace_root: Path,
    parent: ElementRecord,
    mask: Image.Image,
    state: WorkspaceState | None,
) -> tuple[Image.Image, bool]:
    if state is None or parent.assetRole != "parent" or parent.canvas is None:
        return mask, False

    result = normalize_mask(parent.id, mask, mask.size)
    changed = False
    for child in state.elements:
        child_mask = _child_mask_for_parent(workspace_root, parent, child)
        if child_mask is None:
            continue
        result = normalize_mask(
            parent.id,
            ImageChops.multiply(result, ImageChops.invert(child_mask)),
            result.size,
        )
        changed = True
...
def _child_mask_for_parent(
    workspace_root: Path,
    parent: ElementRecord,
    child: ElementRecord,
) -> Image.Image | None:
    if child.canvas is None or parent.canvas is None:
        return None
    if child.assetRole != "removable_child":
        return None
    if child.removeFromParent != parent.id and child.parentId != parent.id:
        return None
    if child.segmentationStatus not in {"mask_suggested", "mask_editing", "mask_accepted"}:
        return None
    if not child.mask:
        return None

    child_path = _workspace_file(workspace_root, child.mask)
    if not child_path.exists():
        return None
    with Image.open(child_path) as child_file:
        child_mask = normalize_mask(child.id, child_file.copy(), (child.canvas.w, child.canvas.h))
    return _project_child_mask_to_parent_canvas(parent, child, child_mask)


def _project_child_mask_to_parent_canvas(
    parent: ElementRecord,
    child: ElementRecord,
    child_mask: Image.Image,
) -> Image.Image | None:
    if parent.canvas is None or child.canvas is None:
        return None
    left = max(parent.canvas.x, child.canvas.x)
    top = max(parent.canvas.y, child.canvas.y)
    right = min(parent.canvas.x + parent.canvas.w, child.canvas.x + child.canvas.w)
    bottom = min(parent.canvas.y + parent.canvas.h, child.canvas.y + child.canvas.h)
    if left >= right or top >= bottom:
        return None

    projected = Image.new("L", (parent.canvas.w, parent.canvas.h), 0)
    crop = child_mask.crop(
        (
            left - child.canvas.x,
            top - child.canvas.y,
            right - child.canvas.x,
            bottom - child.canvas.y,
        )
    )
    # WHY: parent 的最终 mask 代表“父自身可导出的像素”；removable_child 已经作为独立资产，
    # 这里按 canvas 交集扣掉 child mask，避免 wall cabinet 这类父图重复带出植物/瓶子。
    projected.paste(crop, (left - parent.canvas.x, top - parent.canvas.y))
    return projected
