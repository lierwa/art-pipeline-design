...
describe("App sticker workflow wiring 02", () => {
  ...
  it("renders the floating segment drawer in the canvas workspace and wires mask review accept", async () => {
    const user = userEvent.setup();
    ...
    const restoreFetch = installFetchMock(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (input === "/api/workspace/state" && (!init || init.method === "GET")) {
        return jsonResponse(workspace({ elements: [suggestedElement] }));
      }
    ...
    try {
      ...
      expect(screen.getAllByRole("img", { name: /sticker sam2 edge mask/i }).length).toBeGreaterThan(0);
      ...
      expect(screen.queryByRole("button", { name: /export asset pack/i })).not.toBeInTheDocument();
      expect(screen.queryByRole("button", { name: /download pack/i })).not.toBeInTheDocument();

      await user.click(within(drawer).getByRole("button", { name: /accept mask/i }));
      await waitFor(() => {
        expect(globalThis.fetch).toHaveBeenCalledWith(
      ...
      expect(within(topAppBar).queryByRole("button", { name: /generate all final assets/i })).not.toBeInTheDocument();
    } finally {
      restoreFetch();
    }
  });
  ...
  it("keeps the app rail to Upload, Detect, Mask, Generate", async () => {
    const restoreFetch = installFetchMock(async (input: RequestInfo | URL, init?: RequestInit) => {
      if (input === "/api/workspace/runs" && (!init || init.method === "GET")) {
        return jsonResponse({ detail: "legacy mode" }, 500);
      }
